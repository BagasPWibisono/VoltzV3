...
async function getHelpMenu({ client, guild }) {
  ...
  for (const [k, v] of Object.entries(CommandCategory)) {
    ...
    options.push({
      label: v.name,
      value: k,
      description: `Lihat command untuk kategori ${v.name}`,
      emoji: v.emoji,
    });
  }
  ...
  const menuRow = new ActionRowBuilder().addComponents(
    new StringSelectMenuBuilder()
      ...
      .setPlaceholder("Silahkan pilih kategori perintah")
      .addOptions(options)
  );
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      "**Info Bot:**\n" +
        `Hai saya ${guild.members.me.displayName}!\n` +
        "Bot Personal @voltac blablabla (malas buat deskripsi)\n\n"
    );
